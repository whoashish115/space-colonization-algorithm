...
static bool insideWorld(float x, float y, float z) {
    return std::fabs(x) <= WORLD_RADIUS &&
        std::fabs(y) <= WORLD_RADIUS &&
        std::fabs(z) <= WORLD_RADIUS;
}
...
static void drawWireSphre(float radius, int latSegments, int lonSegments,
    ...
    for (int i = 1; i < latSegments; ++i) {
        ...
        for (int j = 0; j < lonSegments; ++j) {
            ...
            glVertex3f(r * std::cos(lon), y, r * std::sin(lon));
        }
        glEnd();
    }
    for (int j = 0; j < lonSegments; ++j) {
...
static void drawSegments() {
    ...
    for (const Segment& s : g_segments) {
        float depthFade = clampf(1.0f - s.depth * 0.00035f, 0.2f, 1.0f);
...
static void drawLeaves() {
    ...
    for (const GlowLayer& layer : layers) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            float hue = hashRange(i, 85.0f, 150.0f);
            float sat = hashRange(i * 7 + 3, 60.0f, 95.0f);
            float r, g, b;
